    ...
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className="border-b border-black pb-7">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <span className={`inline-flex items-center gap-2 rounded-full border ${pal.border} bg-[var(--slot4-surface-bg)] px-4 py-2 text-xs font-extrabold uppercase tracking-[0.18em]`}><BookOpen className="h-4 w-4" /> Article archive</span>
          </div>
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] lg:items-end">
            ...
            <form action={basePath} className="grid gap-3 rounded-xl border border-black/[0.07] bg-white/80 p-3 shadow-sm sm:grid-cols-[1fr_auto] lg:max-w-xl lg:justify-self-end">
              ...
              <button className={`rounded-lg ${pal.darkBg} px-6 py-3 text-sm font-extrabold text-white`}>Filter</button>
            </form>
          </div>
        </div>
        {lead ? (
          <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.75fr)]">
            <Link href={postHref('article', lead, basePath)} className="group grid overflow-hidden rounded-xl border border-black/[0.07] bg-[var(--slot4-surface-bg)] shadow-[0_22px_70px_rgba(31,31,31,0.10)] md:grid-cols-[1.05fr_0.95fr]">
              ...
              <div className="flex min-h-[340px] flex-col justify-between p-6 sm:p-8">
                ...
                <span className="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-extrabold text-white">Start reading <ArrowRight className="h-4 w-4" /></span>
              </div>
            </Link>
            <aside className="rounded-xl border border-black/[0.07] bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
              <div className="flex items-center gap-2 border-b border-black/[0.07] pb-4"><Clock3 className="h-4 w-4 text-[var(--slot4-accent)]" /><h2 className="text-sm font-extrabold uppercase tracking-[0.2em]">Editor shortlist</h2></div>
              {side.map((post, index) => (
                <Link key={post.id || post.slug} href={postHref('article', post, basePath)} className="group grid grid-cols-[76px_minmax(0,1fr)] gap-4 border-b border-black/[0.07] py-4 last:border-b-0">
                  ...
                  <div className="min-w-0"><p className={`${dc.type.eyebrow} ${pal.accentText}`}>Pick {index + 1}</p><h3 className="mt-1 line-clamp-2 text-base font-extrabold leading-tight tracking-tight">{post.title}</h3><p className={`mt-2 line-clamp-2 text-xs font-semibold leading-5 ${pal.mutedText}`}>{getEditableExcerpt(post, 82)}</p></div>
                </Link>
              ))}
            </aside>
          </div>
      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          ...
          <span className={`rounded-xl ${pal.darkBg} px-5 py-3 text-sm font-extrabold text-white`}>Page {page} of {pagination.totalPages || 1}</span>
...
export function EditableArticleDetailShell({ slug, post }: { slug: string; post: SitePost | null }) {
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-xl border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_20px_60px_rgba(24,20,17,0.08)] lg:grid-cols-[minmax(0,1fr)_300px] lg:p-10`}>
          <div className="min-w-0">
            ...
            <h1 className={`mt-4 max-w-4xl text-4xl font-extrabold leading-tight tracking-tight ${pal.panelText} sm:text-5xl lg:text-6xl`}>{post?.title || pagesContent.detailPages.article.fallbackTitle}</h1>
          </div>
      ...
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-xl border ${pal.border} bg-[var(--slot4-surface-bg)] p-6 shadow-[0_20px_60px_rgba(24,20,17,0.08)] sm:p-8 lg:p-10`}>
