      ...
      <main className="mx-auto max-w-[1180px] px-4 py-14 sm:px-6 lg:px-8">
        <section className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl">{pagesContent.contact.title}</h1>
            <p className="mt-5 max-w-2xl text-sm leading-8 opacity-70">{pagesContent.contact.description}</p>
            <div className="mt-8 space-y-4">
              {lanes.map((lane) => (
                <div key={lane.title} className="rounded-xl border border-black/[0.06] bg-white/80 p-5 shadow-sm">
          ...
          <div className="rounded-xl border border-black/[0.06] bg-[var(--slot4-surface-bg)] p-7 shadow-[0_18px_70px_rgba(0,0,0,0.14)]">
            <div className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent-soft)] px-4 py-2 text-xs font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-accent)]"><Mail className="h-4 w-4" /> Editorial inbox</div>
            <h2 className="mt-5 text-2xl font-extrabold tracking-tight">{pagesContent.contact.formTitle}</h2>
