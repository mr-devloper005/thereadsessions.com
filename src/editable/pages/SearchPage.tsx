...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-xl border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-[0_18px_44px_rgba(31,31,31,0.10)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        ...
        <h2 className="mt-4 line-clamp-3 text-2xl font-black leading-tight tracking-tight text-[var(--editable-page-text,#211713)]">{post.title}</h2>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#fff7ee)] text-[var(--editable-page-text,#2f1d16)]">
        <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-xl border border-[var(--editable-border)] bg-white/75 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur md:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-xl border border-[var(--editable-border)] bg-[var(--editable-page-bg,#fff7ee)] p-4 sm:p-5">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                ...
                <select name="task" defaultValue={task} className="rounded-xl border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-black outline-none">
                  <option value="">All article lanes</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-xl bg-[var(--editable-page-text,#2f1d16)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#fff7ee)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-xl border border-dashed border-[var(--editable-border)] bg-white/70 p-10 text-center">
