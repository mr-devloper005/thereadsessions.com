    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)] px-4 py-14 text-[var(--slot4-page-text)] sm:px-6 lg:px-8">
        <section className="mx-auto grid max-w-[1180px] gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          <article className="rounded-xl border border-black/[0.06] bg-white/85 p-8 shadow-sm lg:p-12">
            <p className="text-xs font-extrabold uppercase tracking-[0.24em] opacity-55">{pagesContent.about.badge}</p>
            <h1 className="mt-5 text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl">About {globalContent.site.name}</h1>
          ...
          <aside className="space-y-4">
            {pagesContent.about.values.map((value) => (
              <div key={value.title} className="rounded-xl border border-black/[0.06] bg-white/75 p-6 shadow-sm">
            ...
            <div className="rounded-xl border border-black/[0.06] bg-[var(--slot4-accent-soft)] p-6">
              <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Editorial scope</p>
