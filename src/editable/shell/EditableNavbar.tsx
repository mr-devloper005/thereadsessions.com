    ...
    <header style={navVars} className="sticky top-0 z-50 border-b border-[var(--editable-border)] bg-[var(--editable-nav-bg)]/92 text-[var(--editable-nav-text)] backdrop-blur-2xl">
      <nav className="mx-auto flex min-h-[72px] w-full max-w-[var(--editable-container)] items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          ...
          <span className="hidden min-w-0 sm:block">
            <span className="block max-w-[190px] truncate text-xl font-black tracking-tight">{brandName}</span>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          {session ? (
            <>
              <Link href="/create" className="hidden items-center gap-2 rounded-full bg-[var(--editable-cta-bg)] px-4 py-2.5 text-sm font-black text-[var(--editable-cta-text)] shadow-sm sm:inline-flex"><PlusCircle className="h-4 w-4" /> Write</Link>
              <Link href="/create" className="hidden max-w-[180px] items-center gap-2 truncate rounded-full border border-[var(--editable-border)] bg-white px-3 py-2 text-sm font-black hover:bg-black/5 sm:inline-flex"><UserCircle className="h-4 w-4 shrink-0" /> <span className="truncate">{session.name}</span></Link>
      ...
      {open ? (
        <div className="border-t border-[var(--editable-border)] bg-[var(--editable-nav-bg)] px-4 py-4 lg:hidden">
          ...
          {session ? <div className="mb-3 rounded-xl border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-black">Signed in as {session.name}</div> : null}
